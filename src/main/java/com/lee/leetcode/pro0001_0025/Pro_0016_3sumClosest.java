...
public class Pro_0016_3sumClosest {
	...
	public static int threeSumClosest(int[] nums, int target) {
        int kEnd = nums.length, jEnd = kEnd - 1, iEnd = jEnd - 1;
		int sum = nums[0] + nums[1] + nums[2], diff = Math.abs(target - sum);
		if(kEnd == 3) { return sum; }
        for(int i=0; i<iEnd; i++) {
			for(int j=i+1; j<jEnd; j++) {
				int temp = nums[i] + nums[j], tSum = 0, tDiff = 0;
				for(int k=j+1; k<kEnd; k++) {
					 tSum = temp + nums[k];
					 tDiff = Math.abs(target - tSum);
					 if(tDiff < diff) {
						 diff = tDiff;
						 sum = tSum;
					 }
